CREATE OR REPLACE PROCEDURE sbxtax4."CT_REP_RATEBY"
   ...
   IS
    ...
    'Authority_Name,Authority_Type,Admin_Level,Authority_Category,Rate_Code, Description, Start_Date, End_Date, Rate_type, Amount_Type, Rate, Min_Threshold, Max_Limit, Tier_Rate_Code';
    authTypeId NUMBER;
    zoneLevelId NUMBER;
BEGIN
    ...
    FOR r IN (
        SELECT a.name Authority_Name, aty.name authority_Type, az.name admin_level, a.authority_category, r.rate_code, r.description, r.start_date, r.end_date,
            CASE WHEN l.code IS NULL THEN 'RATE' ELSE l.description END rate_type, lat.description amount_type, NVL(r.rate,rt.rate) rate, NVL(rt.amount_low,0) threshold, rt.amount_high max_limit,
        ...
        LEFT OUTER JOIN tb_lookups l ON (l.code_group = 'SPLIT_TYPE' and l.code = nvl(r.split_type,'x'))
        LEFT OUTER JOIN tb_lookups lat ON (lat.code_group = 'SPLIT_AMT_TYPE' and lat.code = nvl(r.split_amount_type,'x'))
