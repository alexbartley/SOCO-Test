CREATE OR REPLACE PROCEDURE sbxtax4."DATAX_TB_ZONES_152"
   ( taxDataProviderId IN NUMBER, runId IN OUT NUMBER)
   IS
   --<data_check id="152" name="Zones with multiple authorities where one authority is No Content or No VAT">
...
BEGIN
    ...
    AND a.name IN ('No Content', 'No VAT')
    AND NOT EXISTS (
        SELECT 1
