CREATE OR REPLACE PROCEDURE sbxtax4."CT_REPORTS_EXECUTE" (inputParameters IN VARCHAR2, filename OUT VARCHAR2)
   IS
...
BEGIN
   ...
   IF (localParameterString IS NOT NULL) THEN
            ...
            VALUES ('CT_REPORTS_EXECUTE',SYSDATE,'Original parameters:'||substr(localParameterString,1,980));
   ...
   END IF;
       ...
       -- INSERT INTO ct_proc_log(procedure_name, execution_date, message)
      --  VALUES ('CT_REPORTS_EXECUTE',SYSDATE,'file:'||filename);
   ...
   WHILE (pIndex IS NOT NULL) LOOP
        ...
        INSERT INTO ct_proc_log(procedure_name, execution_date, message)
        VALUES ('CT_REPORTS_EXECUTE',SYSDATE,'key:'||pIndex||' '||'value:'||reportParameters(pIndex));
   ...
   INTO reportId, l_filename, procedureCall
   ...
   WHILE (pIndex IS NOT NULL) LOOP
       ...
       -- VALUES ('CT_REPORTS_EXECUTE',SYSDATE,'key:'||pIndex||' '||'value:'||reportParameters(pIndex));
        pIndex := reportParameters.NEXT(pIndex);
   END LOOP;
   procedureCall := REPLACE(procedureCall,'${filename}',''''||l_filename||'''');
   ...
   VALUES (reportId,'QUEUED',SYSDATE, l_filename, SUBSTR(procedureCall,INSTR(procedureCall,'(')+1,LENGTH(procedureCall)-INSTR(procedureCall,'(')-2));
   filename := l_filename;
