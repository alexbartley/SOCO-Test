CREATE OR REPLACE PROCEDURE sbxtax4."GEN_SCENARIOS_STH_PT" (qaUserId IN NUMBER, authority IN VARCHAR2, taxHolidayDate IN VARCHAR2)
   IS
    ...
    stGeo varchar2(100);
    stZip varchar2(100);
    invoiceDate date := to_date(taxHolidayDate, 'dd-MON-yyyy');
    ...
    AND r.start_Date <= taxHolidayDate
    AND r.end_date >= taxHolidayDate
    ...
    AND r.start_Date <= taxHolidayDate
    AND NVL(r.end_date,'31-Dec-9999') >= taxHolidayDate
    AND r.end_date is not null
    ...
    AND NOT EXISTS (
        ...
        AND m.name LIKE 'US - '||SUBSTR(authName,1,2)||' STH'
        ...
        FOR auth IN auths(mer.stateCode) LOOP
            ...
            FOR prod IN prods(auth.authority_id, auth.auth_name) LOOP
        		...
        		INSERT INTO tb_scenarios (
                    ...
                    SUBSTR(auth.auth_name,1,75) || ' - ' || prod.rule_prodcode || ' (US)', 'GS', invoiceDate, qaMerchId, 'S',
                    'F', 'USD', qaUserId, SYSDATE, qaUserId, SYSDATE,
                    stGeo, stZip,stCity, stCounty, stState, 'US',
        		...
        		INSERT INTO tb_scenarios (
                    ...
                    SUBSTR(auth.auth_name,1,75) || ' - ' || prod.rule_prodcode || ' (CU)', 'GS', invoiceDate, qaMerchId, 'B',
                    'F', 'USD', qaUserId, SYSDATE, qaUserId, SYSDATE,
                    stGeo, stZip,stCity, stCounty, stState, 'US'
                    );
                COMMIT;
