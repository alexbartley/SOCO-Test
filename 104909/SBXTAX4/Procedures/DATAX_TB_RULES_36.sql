CREATE OR REPLACE PROCEDURE sbxtax4."DATAX_TB_RULES_36"
   ( taxDataProviderId IN NUMBER, runId IN OUT NUMBER)
   IS
   --<data_check id="36" name="State Level Product Rules without correct (Product Rule Order + .5) Cascading Rule" >
   dataCheckId NUMBER := -743;
BEGIN
    INSERT INTO datax_records (recorded_message, run_id)
    VALUES ('DATAX_TB_RULES_36 started.',runId) RETURNING run_id INTO runId;
    COMMIT;
    ...
    AND NOT EXISTS (
        ...
        AND NVL(r2.no_tax,'N') = NVL(r.no_tax,'N')
        AND NVL(r2.is_local,'N') = 'Y'
        )
    AND NOT EXISTS (
    ...
    INSERT INTO datax_records (recorded_message, run_id)
    VALUES ('DATAX_TB_RULES_36 finished.',runId);
    COMMIT;
END;
