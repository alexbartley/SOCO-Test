CREATE OR REPLACE PROCEDURE sbxtax4."CT_UPDATE_ZONE_TREE"
   IS
   ...
   maxLeaf NUMBER;
   deleteSql VARCHAR2(1000) :='DELETE FROM ct_zone_tree zt WHERE zt.zone_${leafNo}_id IS NOT NULL AND NOT EXISTS (SELECT 1 FROM ct_zone_tree WHERE primary_key = zt.zone_${leafNo}_id)';
   emSql VARCHAR2(2000);
BEGIN
    UPDATE ct_zone_tree
    ...
    COMMIT;
    INSERT INTO CT_PROC_LOG (procedure_name, execution_Date, message)
    VALUES ('CT_UPDATE_ZONE_TREE',executionDate,to_char(affected)||' records deleted from CT_ZONE_TREE because they''ve been modified in TB_ZONES.');
    COMMIT;
    ...
    END LOOP;
    INSERT INTO CT_PROC_LOG (procedure_name, execution_Date, message)
    VALUES ('CT_UPDATE_ZONE_TREE',executionDate,to_char(affected)||' records deleted from CT_ZONE_TREE because affected parent record was removed from CT_ZONE_TREE.');
    COMMIT;
    ...
    COMMIT;
    INSERT INTO CT_PROC_LOG (procedure_name, execution_Date, message)
    VALUES ('CT_UPDATE_ZONE_TREE',executionDate,to_char(affected)||' records deleted from CT_ZONE_AUTHORITIES because the corresponding Zone has been modified in TB_ZONES.');
    COMMIT;
