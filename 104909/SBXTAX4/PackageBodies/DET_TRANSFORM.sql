CREATE OR REPLACE PACKAGE BODY sbxtax4.det_transform
IS
...
/*
  ...
  ALTER TABLE tdr_etl_tb_authorities
  ADD (
...
*/
    ...
    begin
        ...
        -- Changes for CRAPP-3907, 2478 and 3862
        ...
        FOR pt in product_trees LOOP
           ...
           -- BEGIN
            ...
            --EXCEPTION WHEN others THEN
                --dbms_output.put_line(p.prodcode||':'||p.sort_key);
           -- END;
    ...
    PROCEDURE build_tb_rules IS
        ...
        cursor product_exceptions (jta_nkid_i IN VARCHAR2, rate_code_i IN VARCHAR2, hierarchy_level_i IN NUMBER) IS
            ...
            from tdr_etl_product_exceptions rp
    ...
    BEGIN
        ...
        etl_proc_log_p ('DET_TRANSFORM.BUILD_TB_RULES','Updating extract_log not transformed date','RULE',null,null);

        etl_proc_log_p ('DET_TRANSFORM.BUILD_TB_RULES','Removing un processed rules from tmp tables','RULE',null,null);
        ...
        where exists (
            ...
            and (transformed is not null or not_transformed is not null)
            and el.nkid = r.nkid
            and el.rid = r.rid
            );
    ...
    BEGIN
        ...
        IF (upper(package_i) like '%INTERNATIONAL%' or UPPER(package_i) like '%CANADA%' ) THEN -- Changes for CRAPP-2970/2864
            l_content_type := 'INTL';
        ...
        ELSE
            ...
            LOOP
                ...
                UPDATE tdr_etl_authority_base a
                   SET authority_type =
                           NVL ( vauthority_type, REPLACE ( REPLACE (authority_type, 'Service Tax', 'Sales/Use'), '?', effective_zone_level))
        ...
        build_tb_contributing_auths; -- CRAPP-3239
        build_tb_auth_logic;
    ...
    PROCEDURE build_tb_zone_authorities(make_changes_i IN NUMBER) IS  -- 03/24/17 - crapp-3363
        l_id  NUMBER;
        l_rec NUMBER := 0;
        vcurrent_schema VARCHAR2(50); -- Changes for CRAPP-3174 11/01
    ...
    BEGIN
        ...
        SELECT SYS_CONTEXT( 'userenv', 'current_schema' ) INTO vcurrent_schema FROM dual;  -- Changes for CRAPP-3174 11/01
        ...
        DBMS_STATS.gather_table_stats(vcurrent_schema, 'tdr_etl_ct_zone_authorities', cascade => TRUE); -- Changes for CRAPP-3174 11/01
