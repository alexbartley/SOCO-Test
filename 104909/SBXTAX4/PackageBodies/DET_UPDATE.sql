CREATE OR REPLACE PACKAGE BODY sbxtax4.det_update
is
...
*/
   ...
   PROCEDURE compare_authority_logic(make_changes_i IN NUMBER)
    ...
    SELECT DISTINCT
        ...
        end_date,
        process_order
    FROM tdr_etl_tb_auth_logic_groups
    MINUS
    SELECT
        a.uuid,
   ...
   exception
    ...
    BEGIN
        ...
        ELSE
            ...
            SET end_date = end_Date_i,
                last_update_date = sysdate,
                last_updated_by = -1703-- CRAPP-3239
    ...
    BEGIN
        ...
        dbms_output.put_line('rule_id_i value is(blank indicates new rule, else existing rule update) '||rule_id_i);
        ...
        dbms_output.put_line('auth_id_i value is '||auth_id_i);
        dbms_output.put_line('rule_order_i value is '||rule_order_i);
...
PROCEDURE update_rate(
    ...
    EXCEPTION
        WHEN others THEN
        ROLLBACK;
        log_failure(SQLCODE||': '||SQLERRM, 'TB_RATES', l_record.rate_id, l_current_action||' (authority_uuid_i='||authority_uuid_i||' '||rate_Code_i||')');
    RAISE_APPLICATION_ERROR(-20002,'Update rates error.'||l_current_action||' (authority_uuid_i='||authority_uuid_i||' '||rate_Code_i||')');
