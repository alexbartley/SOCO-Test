CREATE OR REPLACE PACKAGE BODY sbxtax.cr_extract
IS
...
/*
   ...
   ALTER TABLE SBXTAX.tdr_etl_authority_base
   ADD (
...
*/
    ...
    BEGIn
        ...
        DELETE FROM tdr_etl_jta_inv_desc tid  -- CRAPP-3174, Removed schema reference
        WHERE NOT EXISTS (
            SELECT 1
            FROM tdr_etl_rules r
    ...
    BEGIN
        ...
        INSERT INTO tdr_etl_product_categories (name, description, prodcode, sort_key, nkid,rid,product_tree) (
            SELECT DISTINCT substr(c.name,1,100), substr(c.description, 1, 250), substr(c.commodity_code,1,50), c.h_code, c.nkid, c.rid, c.product_tree_short_name --njv CRAPP-864
    ...
    exception
    when others then
    etl_proc_log_p('CR_EXTRACT.INSERT_CONTRIBUTING_AUTHORITY',SQLERRM,'INSERT_CONTRIBUTING_AUTHORITY',nkid_i,rid_i);
...
/*
exception
Changed this to the caller of the procedure since exception details are not available yet
*/
    ...
    begin
        ...
        DBMS_OUTPUT.Put_Line('CRAPP-3631 Completed');
    end;
...
is
        ...
        from tdr_etl_rule_app_diffs;  -- CRAPP-3174, Removed schema reference
...
begin
    ...
    -- todo: each section might need its own exception and errnum.
    Exception
    when others then
...
PROCEDURE product_taxability_data_2
    ...
    BEGIN
        ...
        product_taxability_data_2;
            ...
            INSERT INTO tdr_etl_rules (authority_uuid,  -- CRAPP-3174, Removed schema reference
                                   nkid,
                                   calculation_method,
                                   basis_percent,
                                   recoverable_percent,
        ...
        */
        ...
        INSERT INTO tdr_etl_jta_inv_desc (authority_uuid,
            ...
            (SELECT DISTINCT authority_uuid,
                             ...
                             d.start_date,
                             tat_id
               FROM tdr_etl_rule_app_diffs d  -- CRAPP-3174, Removed schema reference
                    JOIN content_repo.mv_juris_tax_applicabilities jta
                        ON (jta.nkid = d.jta_nkid)
                    JOIN mp_juris_auths ja ON (ja.nkid = jta.jurisdiction_nkid));

        etl_proc_log_p('CR_EXTRACT.RULE_DATA_2','Start loading qualifiers','Rule Data',NULL,NULL);
        INSERT INTO tdr_etl_rule_qualifiers (authority_uuid,  -- CRAPP-3174, Removed schema reference
                                     end_date,
                                     jta_nkid,
                                     logical_qualifier,
                                     nkid,
    ...
    --            a.tas_nkid
       ...
       etl_proc_log_p('CR_EXTRACT.RULE_DATA_2','Start deleteing unnecessary products','Rule Data',NULL,NULL);
        DELETE FROM tdr_etl_rule_products r  -- CRAPP-3174, Removed schema reference
         WHERE     no_tax = 'N'
               AND exempt = 'N'
               AND EXISTS
                       (SELECT 1
    ...
    BEGIN
        ...
        ELSIF (entity_i = 'REFERENCE GROUP') THEN
            etl_proc_log_p('CR_EXTRACT.LOCAL_EXTRACT','CR_EXTRACT.REFERENCE_DATA Started',entity_i,null,null);
            reference_data;
        ...
        /*
        -- This is now part of GIS_ETL package and no longer needed here.
    ...
    BEGIN
        ...
        ELSIF (entity_i = 'RULES') THEN
            ...
            --delete from tmp_rule_taxes WHERE extract_id = etl_id_i;
...
-- Possible timeout_on_resource
    ...
    */
    ...
    /*
    PROCEDURE compliance_area_data IS -- 03/24/17 - crapp-3363
    ...
    BEGIN
        ...
        etl_proc_log_p('CR_EXTRACT.COMPLIANCE_AREA_DATA', 'Get compliance_areas deletes - start, - '||l_stcode||'', 'GIS', NULL, NULL);
        ...
        etl_proc_log_p('CR_EXTRACT.COMPLIANCE_AREA_DATA', 'Get compliance_areas deletes - end, - '||l_stcode||'', 'GIS', NULL, NULL);


        -- Get Updates --

        etl_proc_log_p('CR_EXTRACT.COMPLIANCE_AREA_DATA', 'Get compliance_areas updates - start, - '||l_stcode||'', 'GIS', NULL, NULL);
        INSERT INTO tdr_etl_us_comp_area_changes
            (id, name, area_uuid, eff_zone_level_id, area_count, start_date, end_date, source_db, change_type, state_code)
            SELECT  l_id+rownum, det.name, det.compliance_area_uuid
                    , tdr.effective_zone_level_id
                    , tdr.associated_area_count
                    , NVL(tdr.tax_areaid_startdate, tdr.start_date) start_date
                    , tdr.end_date
                    , 'DETERMINATION', 'Update', l_stcode
        ...
        INTO l_id
        FROM tb_compliance_areas;
        etl_proc_log_p('CR_EXTRACT.COMPLIANCE_AREA_DATA', 'Get compliance_areas updates - end, - '||l_stcode||'', 'GIS', NULL, NULL);


        -- Get Adds -- Compliance Areas that exist in Content Repo but not in Determination

        etl_proc_log_p('CR_EXTRACT.COMPLIANCE_AREA_DATA', 'Get compliance_areas adds - start, - '||l_stcode||'', 'GIS', NULL, NULL);
        INSERT INTO tdr_etl_us_comp_area_changes
            (id, name, area_uuid, eff_zone_level_id, area_count, start_date, source_db, change_type, state_code)
            -- 09/23/16 added l_id+
            SELECT l_id+rownum, name, compliance_area_uuid, effective_zone_level_id, associated_area_count, start_date, 'CONTENT_REPO', 'Add', l_stcode
            FROM (
                   SELECT  area_id name
        ...
        COMMIT;
        etl_proc_log_p('CR_EXTRACT.COMPLIANCE_AREA_DATA', 'Get compliance_areas adds - end, - '||l_stcode||'', 'GIS', NULL, NULL);
    ...
    PROCEDURE compliance_area_auth_data(make_changes_i IN NUMBER) IS    -- 03/24/17 - crapp-3363
        l_stcode VARCHAR2(2 CHAR);
        l_fips   VARCHAR2(2 CHAR);
        l_id     NUMBER;
    BEGIN
        ...
        etl_proc_log_p('CR_EXTRACT.COMPLIANCE_AREA_AUTH_DATA', 'Get compliance_areas_auth adds - start, - '||l_stcode||'', 'GIS', NULL, NULL);
        ...
        INSERT INTO tdr_etl_us_comp_area_auth_chgs
            ...
            SELECT rownum, compliance_area_uuid, authority_name, ta.authority_id, 'CONTENT_REPO', 'Add'
            FROM (
                   SELECT  NVL(TO_CHAR(tax_area_id), area_id) compliance_area_uuid
                           , authority_name
                           --, area_id  -- 07/18/17, removed
                   FROM    content_repo.gis_tb_comp_area_authorities    -- crapp-3363
                   WHERE   area_id NOT IN (SELECT DISTINCT name FROM tdr_etl_us_comp_area_changes WHERE end_date IS NOT NULL) -- excluded End-Dated areas -- CRAPP-3174, Removed schema reference
                   MINUS
                   SELECT  tca.compliance_area_uuid
                           , ta.NAME authority_name
                           --, tca.NAME -- 07/18/17, removed
        ...
        etl_proc_log_p('CR_EXTRACT.COMPLIANCE_AREA_AUTH_DATA', 'Get compliance_areas_auth adds - end, - '||l_stcode||'', 'GIS', NULL, NULL);


        -- Get Deletes --

        etl_proc_log_p('CR_EXTRACT.COMPLIANCE_AREA_AUTH_DATA', 'Get compliance_areas_auth deletes - start, - '||l_stcode||'', 'GIS', NULL, NULL);
        INSERT INTO tdr_etl_us_comp_area_auth_chgs
            (id, area_uuid, authority_name, authority_id, source_db, change_type)
            SELECT caa.compliance_area_auth_id, caa.compliance_area_id, ta.name, ta.authority_id, 'DETERMINATION', 'Delete' --l_id+rownum
            FROM   tb_comp_area_authorities caa
                   JOIN tb_authorities ta ON (caa.authority_id = ta.authority_id)
                   JOIN (
        ...
        COMMIT;
        etl_proc_log_p('CR_EXTRACT.COMPLIANCE_AREA_AUTH_DATA', 'Get compliance_areas_auth deletes - end, - '||l_stcode||'', 'GIS', NULL, NULL);
