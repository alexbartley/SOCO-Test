CREATE OR REPLACE procedure sbxtax2.rate_extract(filename IN varchar2, extract_date IN varchar2) is
...
from (
select  full_address.zone_2_name Country, full_address.zone_3_name State_Province, full_address.zone_4_name County, full_address.zone_5_name City, full_address.zone_6_name ZipCode, full_address.zone_7_name GeoCode, 
...
join ct_Zone_authorities za on (
    ...
    or za.primary_key = nvl(full_address.zone_6_id ,-1)
    or za.primary_key = nvl(full_address.zone_5_id ,-1)
    or za.primary_key = full_address.zone_4_id
    or za.primary_key = full_address.zone_3_id
    or za.primary_key = full_address.zone_2_id
    )
join tb_authorities auth on (auth.name = za.authority_name and auth.merchant_id = za.merchant_id and UPPER(auth.name) not like '%REVERSE CHARGE%')
...
where m.name = 'Sabrix INTL Tax Data'
...
group by full_address.zone_2_name, full_address.zone_3_name, full_address.zone_4_name, full_address.zone_5_name, full_address.zone_6_name, full_address.zone_7_name,
