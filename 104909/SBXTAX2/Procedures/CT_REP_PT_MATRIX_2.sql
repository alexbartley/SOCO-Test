CREATE OR REPLACE PROCEDURE sbxtax2.ct_rep_pt_matrix_2
   ( taxDataProvider IN VARCHAR2, productGroup IN VARCHAR2, authorityGroup IN VARCHAR2, authorityKeyword IN VARCHAR2, filename IN VARCHAR2)
   IS
...
--SQL statement for updating Tax Type specific taxability
sqlTaxType VARCHAR2(4000) :=
    'MERGE INTO pt_temp_matrix
        USING  (
            ...
            AND pt.tax_type IS NOT NULL
            AND pt.rate_code IS NULL
            ) auth_pt
        ON (pt_temp_matrix.primary_key = auth_pt.primary_key)
        WHEN MATCHED THEN UPDATE SET ${authorityColumn} = ''E'' WHERE ${authorityColumn} IS NULL' ; 
--SQL statement for updating Any Tax Type (only where not populated by 'Tax Type specific taxability')
sqlNoTaxType VARCHAR2(4000) :=
    'MERGE INTO pt_temp_matrix
        USING  (
            SELECT DISTINCT pt.primary_key
            FROM pt_product_taxability pt
...
--SQL statement for updating the remainder to Catch-All(No Product attachment) Tax Type Specific Rule 
sqlTaxTypeNoProduct VARCHAR2(4000) :=
...
--SQL statement for updating the remainder to Catch-All(No Product attachment) Any Tax Type Rule 
sqlNoTaxTypeNoProduct VARCHAR2(4000) :=
    'UPDATE pt_temp_matrix ptm
    SET ${authorityColumn} = ''E''
    WHERE ${authorityColumn} IS NULL
    AND EXISTS (
...
--SQL statement for updating 'Taxability' that doesn't get caught by any Rule
...
BEGIN
    ...
    --If CT_ANALYZE_PRODUCT_TAXABILITY is currently running, wait until it is finished to avoid dirty reads
    ...
    --Update the status for this report in the Report Queue
    ...
    --Get the Content_Version to determine if this TDP may be Canada
    SELECT content_version
    INTO contentVersion
    FROM tb_merchants
    WHERE name = taxDataProvider;

    --Get the Product_Group_Id for the user-selected Product Group
    ...
    /*
        The following IF/ELSE block determines the list of Authorities to produce the PT Matrix for,
        and populates PT_TEMP_AUTHORITY_COLUMNS with those Authorities
    */
    --If the user specified Sabrix US Tax Data and All States and did not provide an AuthorityKeyword
    --AuthorityKeyword is checked because the AuthorityGroup may not have been selected
    IF taxDataProvider LIKE '%US Tax%' AND NVL(authorityGroup,'All States') = 'All States' AND authorityKeyword IS NULL THEN
        ...
        --This is the TaxResearch-defined criteria for 'All States'
        FOR a IN (
    ...
    --If the user specified Sabrix INTL Tax Data and All Countries and did not provide an AuthorityKeyword
    --AuthorityKeyword is checked because the AuthorityGroup may not have been selected
    ELSIF taxDataProvider LIKE '%INTL Tax%' AND NVL(authorityGroup,'All Countries') = 'All Countries' AND authorityKeyword IS NULL THEN
    ...
    --If the user specified All Provinces
    ELSIF authorityGroup = 'All Provinces' THEN
        --If Canada Tax Data then use Canada Product Group as default
        IF contentVersion LIKE '%CANADA%' THEN
            productGroupId := NVL(productGroupId,-1);
        --If MTS INTL Tax Data then use UNSPSC Product Group as default
        ELSIF contentVersion LIKE '%MTS' THEN
            productGroupId := NVL(productGroupId,-999);
        --Otherwise use Harmonized Product Group
        ELSE 
            productGroupId := NVL(productGroupId,-3); --Set to default Harmonized Product Group
        END IF;
    ...
    --If the user specified EU Countries
    ...
    --If the user specified an AuthorityKeyword
    ...
    --200 Authorities is the limit because that's how many columns are in PT_TEMP_AUTHORITY_COLUMNS
    --Given the huge-ness of the matrix if there are more than 50 or so Authorities, the usefulness of the PT Matrix decreases as the number of Authorities increases.
    --Also, the method of writing the output will overflow the buffer if there are 'too many' Authorities, which has been determined to be somewhere between 200 and 500
    IF aIndex > 200 THEN
        RAISE too_many_authorities;
    END IF;
    
    --Put all the 'Ordered' Products into PT_TEMP_MATRIX
    ...
    --Populate the Taxability by Authority, one Authority at a time
    FOR ac IN (
        ...
        --The purpose of the PT Matrix is to show the lowest level/ most conditionals for a Product/Authority (excluding Tax Code Rules)
        --For this report, Exempts supersedes Taxables (i.e. if a Product is Exempt in any way, show it as Exempt) and Tax Type supersedes No Tax Type
        
        --The order of execution of these updates is essential
        --1: Set the taxability that is specific to a Product Rule WITH a Tax Type condition
        executeMergePtSql := replace(replace(replace(sqlTaxType,'${authorityId}',ac.authority_id),'${productGroupId}',productGroupId),'${authorityColumn}',ac.column_name);  
        execute immediate executeMergePtSql;
        --2: Set the taxability that is specific to a Product Rule WITHOUT a Tax Type condition
        executeMergePtSql := replace(replace(replace(sqlNoTaxType,'${authorityId}',ac.authority_id),'${productGroupId}',productGroupId),'${authorityColumn}',ac.column_name);  
        execute immediate executeMergePtSql;
        --3: Set the Exempts that are NOT specific to a Product Rule WITH a Tax Type condition
        executeMergePtSql := replace(replace(replace(sqlTaxTypeNoProduct,'${authorityId}',ac.authority_id),'${productGroupId}',productGroupId),'${authorityColumn}',ac.column_name);  
        execute immediate executeMergePtSql;
        --4: Set the Exempts that are NOT specific to a Product Rule WITHOUT a Tax Type condition
        executeMergePtSql := replace(replace(replace(sqlNoTaxTypeNoProduct,'${authorityId}',ac.authority_id),'${productGroupId}',productGroupId),'${authorityColumn}',ac.column_name);  
        execute immediate executeMergePtSql;
        --5: Update everything else to Taxable
