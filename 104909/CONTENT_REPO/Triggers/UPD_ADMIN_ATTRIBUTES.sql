CREATE OR REPLACE TRIGGER content_repo.UPD_ADMIN_ATTRIBUTES
 FOR 
 UPDATE
...
COMPOUND TRIGGER
    ...
    TYPE mod_records IS TABLE OF administrator_attributes%ROWTYPE;
    pending_changes mod_records := mod_records(); --collection of record updates in this transaction

    BEFORE EACH ROW IS
        l_new administrator_attributes%ROWTYPE;
    ...
    BEGIN
        ...
        IF NOT l_changed AND NVL(:new.status,:old.status) != :old.status THEN
            IF (:new.administrator_id != :old.administrator_id OR :new.attribute_id != :old.attribute_id) THEN
        ...
        ELSE
            ...
            IF (l_new.rid != :old.rid) THEN
                ...
                l_new.id := pk_Administrator_attributes.nextval; --new ID
            ...
            ELSE
                ...
                WHERE table_name = 'ADMINISTRATOR_ATTRIBUTES'
