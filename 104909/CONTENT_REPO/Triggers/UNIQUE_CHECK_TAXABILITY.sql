CREATE OR REPLACE TRIGGER content_repo."UNIQUE_CHECK_TAXABILITY"
 FOR
  INSERT OR UPDATE
...
DISABLE COMPOUND TRIGGER
    ...
    BEGIN
        IF l_pcc > 0 THEN
            ...
            taxability.unique_check(pending_changes(r).jurisdiction_nkid,pending_changes(r).reference_code,pending_changes(r).nkid);
            END LOOP;
        END IF;
    EXCEPTION
        WHEN others THEN
