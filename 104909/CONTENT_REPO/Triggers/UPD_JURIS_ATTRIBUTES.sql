CREATE OR REPLACE TRIGGER content_repo.UPD_JURIS_ATTRIBUTES
 FOR 
 UPDATE
...
COMPOUND TRIGGER
    ...
    TYPE mod_records IS TABLE OF jurisdiction_attributes%ROWTYPE;
    pending_changes mod_records := mod_records(); --collection of record updates in this transaction

    BEFORE EACH ROW IS
        l_new jurisdiction_attributes%ROWTYPE;
    ...
    BEGIN
        ...
        ELSIF l_changed THEN
            IF (:new.jurisdiction_id != :old.jurisdiction_id OR :new.attribute_id != :old.attribute_id) THEN
                RAISE errnums.cannot_update_record;
            END IF;
            ...
            IF (l_new.rid != :old.rid) THEN
                ...
                l_new.id := pk_jurisdiction_attributes.nextval; --new ID
            ...
            ELSE
                ...
                WHERE table_name = 'JURISDICTION_ATTRIBUTES'
