CREATE OR REPLACE TRIGGER content_repo."UPD_TAX_APP_ATTRIBUTES" 
...
COMPOUND TRIGGER
    ...
    TYPE mod_records IS TABLE OF JURIS_TAX_APP_ATTRIBUTES%ROWTYPE;
    pending_changes mod_records := mod_records(); --collection of record updates in this transaction

    BEFORE EACH ROW IS
        l_new JURIS_TAX_APP_ATTRIBUTES%ROWTYPE;
        --l_pci NUMBER;
        l_changed BOOLEAN;
    BEGIN
        ...
        ELSIF l_changed THEN
            IF (:new.JURIS_TAX_APPLICABILITY_ID != :old.JURIS_TAX_APPLICABILITY_ID OR :new.attribute_id != :old.attribute_id) THEN
                RAISE errnums.cannot_update_record;
            END IF;
            ...
            IF (l_new.rid != :old.rid) THEN
                ...
                l_new.id := pk_juris_TAX_APP_ATTRIBUTES.nextval; --new ID
            ...
            ELSE
                ...
                WHERE table_name = 'JURIS_TAX_APP_ATTRIBUTES'
