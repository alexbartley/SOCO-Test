CREATE OR REPLACE FORCE VIEW content_repo.vunique_area_attributes_kpmg ("ID",nkid,rid,next_rid,unique_area_id,unique_area_nkid,unique_area_rid,unique_area_next_rid,attribute_category,attribute_category_id,"VALUE",value_id,attribute_name,attribute_id,start_date,end_date,status,status_modified_date,entered_by,entered_date) AS
SELECT guaa.id,
       guaa.nkid,
       guaa.rid,
       guaa.next_rid,
  ...
  FROM GEO_UNIQUE_AREA_ATTRIBUTES guaa
       ...
       JOIN GEO_UNIQUE_AREA_revisions guar
          ON (    guar.nkid = uai.nkid
              AND rev_join (guaa.rid,
                            guar.id,
                            COALESCE (guaa.next_rid, 99999999)) = 1)
       ...
       LEFT JOIN jurisdictions j
          ON (guaa.VALUE = j.nkid AND j.next_rid IS NULL);