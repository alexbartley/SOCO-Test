CREATE OR REPLACE FORCE VIEW content_repo.vgeo_unique_area_search (state_code,unique_area_id,unique_area,start_date,end_date,rid,nkid,next_rid) AS
(SELECT /*+index(u geo_unique_area_polygons_n1) index(a geo_unique_areas_pk) index(p geo_polygons_pk)*/
      ...
      FROM geo_unique_area_polygons u
           ...
           JOIN (SELECT DISTINCT state_code, area_id,              -- HASH_MD4
                                                     unique_area
                   FROM vgeo_unique_areas2) ua
              ON (a.area_id = ua.area_id) --AND SUBSTR(p.geo_area_key, 1, 2) = ua.state_code)
                                         )
