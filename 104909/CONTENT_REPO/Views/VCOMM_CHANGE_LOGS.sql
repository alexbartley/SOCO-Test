CREATE OR REPLACE FORCE VIEW content_repo.vcomm_change_logs ("ID",nkid,rid,primary_key,is_published,status,status_modified_date,entered_date,entered_by,table_name,change_reason,change_reason_id,change_summary,document_count) AS
SELECT clo.id,
            ar.nkid,
            ar.id,
            clo.primary_key,
       ...
       join comm_qr q on (q.table_name = clo.table_name and q.ref_id = clo.primary_key)
            JOIN entity_table_map etm
               ON (etm.table_name = clo.table_name)
            JOIN commodity_revisions ar
               ON (ar.id = clo.rid)
      ...
      WHERE logical_entity = 'Commodities'
   GROUP BY clo.id,
            ar.nkid,
            ar.id,
            clo.primary_key,
