CREATE OR REPLACE FORCE VIEW content_repo.vtax_search_gis_nogeo (official_name,juris_tax_entity_rid,juris_tax_id,juris_tax_nkid,juris_tax_next_rid,tax_rid,tax_next_rid,jurisdiction_rid,out_rid,out_next_rid,def_rid,def_next_rid,reference_code,start_date,end_date,taxation_type_id,taxation_type,spec_applicability_type_id,specific_applicability_type,transaction_type,transaction_type_id,revenue_purpose_description,revenue_purpose_id,tax_calc_structure_id,tax_structure,amount_type,tax_value_type,referenced_code,tax_value,min_threshold,max_limit,administrator_name,admin_rid,reporting_code,jurisdiction_nkid,ref_juris_tax_rid,state_code) AS
SELECT
    jti.official_name,
    ...
    td2.next_rid,
    jti.reference_code,
    --tou.start_date, --to_date(tou.start_date,'MM/DD/YYYY'),   -- removed 02/26/15 - dlg crapp-
    -- 1314
    --tou.end_date,   --to_date(tou.end_date,'MM/DD/YYYY'),     -- removed 02/26/15 - dlg crapp-
    -- 1314
...
FROM
    vjuris_tax_impositions jti
JOIN
    vtax_outlines tou
ON
    (
        tou.juris_tax_rid = jti.JURIS_TAX_ENTITY_RID)
JOIN
    vtax_calc_structures tcs
ON
    (
        tou.calculation_structure_id = tcs.id)
JOIN
    vtax_descriptions td
ON
    (
        td.id = jti.tax_description_id)
JOIN
    vtax_definitions2 td2
ON
    (
...
ON
    (
        rp.id = NVL (jti.revenue_purpose_id, -1))
LEFT OUTER JOIN
    vjuris_tax_impositions jtr
ON
    (
        td2.ref_juris_tax_id = jtr.id)
LEFT JOIN
    vtax_outlines tou2
ON
    (
        tou2.juris_tax_rid = jtr.JURIS_TAX_ENTITY_RID
    AND tou.START_DATE > tou2.START_DATE
    AND NVL(tou2.END_DATE, '31-DEC-2099') > NVL(tou.END_DATE, '31-DEC-2098') )
LEFT JOIN
    vtax_definitions2 td3
ON
    (
        TD3.JURIS_TAX_RID = tou2.JURIS_TAX_RID
    AND TD3.TAX_OUTLINE_NKID = tou2.NKID )
WHERE
    jti.juris_tax_next_rid IS NULL
