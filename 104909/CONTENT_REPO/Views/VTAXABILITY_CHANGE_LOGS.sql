CREATE OR REPLACE FORCE VIEW content_repo.vtaxability_change_logs ("ID",nkid,rid,primary_key,status,status_modified_date,entered_date,entered_by,entered_by_id,table_name,change_reason,change_reason_id,assignment_type,assignment_type_id,juris_tax,change_summary,document_count) AS
SELECT jtcl.id,
            jtr.nkid,
            jtr.id,
            jtcl.primary_key,
       ...
       join JURIS_TAX_APP_qr q on (q.table_name = jtcl.table_name and q.ref_id = jtcl.primary_key)
            ...
            LEFT JOIN juris_tax_applicabilities jti
               ...
               --ON (jti.rid = jtcl.rid)
               /* changed to rid on 2/11/14 to work around issues of entity_id not being set correctly on some records*/
