CREATE OR REPLACE FORCE VIEW content_repo.vgeo_poly_change_logs ("ID",nkid,rid,primary_key,is_published,status,status_modified_date,entered_date,entered_by,table_name,change_reason,change_reason_id,change_summary,document_count) AS
(SELECT clo.id,
            ar.nkid,
            ar.id,
            clo.primary_key,
       ...
       FROM geo_poly_ref_chg_logs clo
            ...
            JOIN geo_poly_ref_revisions ar
               ON (ar.id = clo.rid)
            LEFT OUTER JOIN geo_poly_ref_chg_cits cc
               ON (cc.geo_poly_ref_chg_log_id = clo.id)
            LEFT OUTER JOIN citations ci
               ON (cc.citation_id = ci.id)
            LEFT OUTER JOIN change_reasons cr
               ON (cr.id = clo.reason_id)
      WHERE logical_entity = 'Boundaries'
   GROUP BY clo.id,
            ar.nkid,
            ar.id,
            clo.primary_key,
