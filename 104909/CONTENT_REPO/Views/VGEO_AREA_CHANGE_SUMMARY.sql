CREATE OR REPLACE FORCE VIEW content_repo.vgeo_area_change_summary ("ID",rid,nkid,status,published,modified,entered_by,table_name,reason,"SUMMARY",documents,veriftype) AS
(SELECT JCL.ID,
            JR.ID,
            JR.NKID,
            JCL.STATUS,
       ...
       FROM geo_unique_area_revisions JR
            JOIN geo_unique_area_chg_logs JCL
               ON JR.ID = JCL.RID
            left join geo_unique_area_chg_vlds vld on (vld.geo_unique_area_chg_log_id = jcl.id)
            JOIN ENTITY_TABLE_MAP ETM
               ON JCL.TABLE_NAME = ETM.TABLE_NAME
            LEFT OUTER JOIN CHANGE_REASONS CR
               ON JCL.REASON_ID = CR.ID
            LEFT OUTER JOIN geo_unique_area_chg_cits JCC
               ON JCL.ID = JCC.geo_unique_area_chg_log_id
