CREATE OR REPLACE PROCEDURE sbxtax3.ct_rep_pt_matrix
   ( taxDataProvider IN VARCHAR2, productGroup IN VARCHAR2, authorityGroup IN VARCHAR2, matrixType IN VARCHAR2, zone IN VARCHAR2, filename IN VARCHAR2)
   IS
...
updateNATaxability VARCHAR2(4000) := 
    ...
    SET ${authorityColumn} = ''N/A''
    WHERE ${authorityColumn} IS NULL';
--SQL statements for showing Rate Code Taxability
updateDefaultPtRateCode VARCHAR2(4000) := 
    'UPDATE pt_temp_matrix
    SET ${authorityColumn} = (
        SELECT NVL(taxability,''N/A'') taxability
        FROM pt_catch_all ca
        WHERE ca.product_group_id = ${productGroupId}
        AND ca.tax_type IS NULL
...
updateDefaultPtTE VARCHAR2(4000) := 
    'UPDATE pt_temp_matrix
    SET ${authorityColumn} = (
        SELECT CASE WHEN NVL(taxability,''T'') NOT IN (''E'',''N'') THEN ''T'' ELSE NVL(taxability,''N/A'') END taxability
        FROM pt_catch_all ca
        WHERE ca.product_group_id = ${productGroupId}
        AND ca.tax_type IS NULL
        AND ca.authority = ''${authority}'')
...
BEGIN
    ...
    ELSE
        updateDefaultPt := updateDefaultPtTE;
        mergePtStatement := mergePtStatementTE;  
    END IF;
    ...
    IF taxDataProvider LIKE '%US Tax%' AND NVL(authorityGroup,'All States') = 'All States' THEN
    ...
    FOR ac IN (
        ...
        executeUpdateSql := replace(replace(replace(updateDefaultPt,'${authority}',ac.authority_name),'${productGroupId}',productGroupId),'${authorityColumn}',ac.column_name);  
    ...
    FOR r IN (
        SELECT sort_key, LPAD(' ',LENGTH(REPLACE(REPLACE(sort_key,'.00000'),'00000')),' ')||product_name product_name, ps.prodcode, tm.*
