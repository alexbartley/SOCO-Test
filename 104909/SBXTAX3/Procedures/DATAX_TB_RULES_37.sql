CREATE OR REPLACE PROCEDURE sbxtax3.datax_tb_rules_37
   ( taxDataProviderId IN NUMBER, runId IN OUT NUMBER)
   IS
   --<data_check id="37" name="Cascading Rules without corresponding Non-Cascading Rule" >
   dataCheckId NUMBER := -784;
BEGIN
    INSERT INTO datax_records (recorded_message, run_id)
    VALUES ('DATAX_TB_RULES_37 started.',runId) RETURNING run_id INTO runId;
    COMMIT;
    ...
    AND NOT EXISTS (
        ...
        AND NVL(sr.rate_code,NVL(sr.exempt,NVL(sr.no_tax,'N'))) = NVL(cr.rate_code,NVL(cr.exempt,NVL(cr.no_tax,'N')))
    )
    AND NOT EXISTS (
        SELECT 1
    ...
    INSERT INTO datax_records (recorded_message, run_id)
    VALUES ('DATAX_TB_RULES_37 finished.',runId);
    COMMIT;
END;
