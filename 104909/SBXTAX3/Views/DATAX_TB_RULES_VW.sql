...
SELECT c.data_Check_id,
         ...
         lc.description calculation_method,
         lt.description tax_type,
         basis_percent,
         exempt_reason_code,
         r.code tax_code,
         pc.name product_name,
         pc.prodcode product_commodity_Code,
         r.last_update_date,
         primary_key
    FROM tb_authorities a
         JOIN tb_rules r ON (a.authority_id = r.authority_id)
         JOIN tb_lookups lc
            ON (    lc.code = r.calculation_method
                AND lc.code_Group = 'TBI_CALC_METH')
         LEFT OUTER JOIN tb_lookups lt
            ON (lt.code = r.tax_type AND lt.code_Group = 'US_TAX_TYPE')
         LEFT OUTER JOIN tb_product_Categories pc
            ON (pc.product_category_id = NVL (r.product_category_id, -222222))
